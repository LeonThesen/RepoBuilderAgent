...
Your task is to:
  ...
  4. Follow these additional guidelines:
     ...
     - Prefer the repo's OWN canonical build over inventing one — the SIMPLEST path that produces the artifact, since every extra step is another failure mode. Use the repo's build wrapper when present (`./gradlew`, `./mvnw`) instead of installing that tool yourself; use the build system its manifest declares (CMake when `CMakeLists.txt` exists, Cargo for `Cargo.toml`, Meson for `meson.build`) and do NOT switch to a different one (e.g. an autotools `./configure` when a `CMakeLists.txt` is present). Build the library/application itself — never its tests, examples, or benchmarks.
