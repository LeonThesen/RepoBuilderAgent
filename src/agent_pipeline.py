...
parser.add_argument(
    ...
    choices=["flat_baseline", "exploration", "synthesis", "validation", "full_system", "ab_prev_attempt_ctx_on", "ab_prev_attempt_ctx_off", "one_shot_direct"],
...
def resolve_variant_policy() -> dict:
    if args.variant == "ab_prev_attempt_ctx_off":
        return {
            "phase2_anchor": False,
...
def main() -> int:
    ...
    if args.variant == "ab_prev_attempt_ctx_off":
        if not args.stateful_repair:
            log_info("Variant ab_prev_attempt_ctx_off selected: forcing stateful repair ON for AB-19B.")
        if args.stateful_repair_tree:
            log_info("Variant ab_prev_attempt_ctx_off selected: forcing stateful repair tree OFF for AB-19B isolation.")
        args.stateful_repair = True
        args.stateful_repair_tree = False
